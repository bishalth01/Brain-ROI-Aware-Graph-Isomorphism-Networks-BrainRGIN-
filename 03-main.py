...
from net.dynamicbraingngaro import CustomNetworkWithGARO
from net.dynamicbrainginwithmean import CustomNetworkGINWithMean
...
wandb.init(project='abcd_fluid_prediction_publish', save_code=True, config="wandb_sweeps/cross_validation/abcd_rgcn.yaml")
...
# # sys.exit()
train_mse_list, train_mae_list, train_r_list = [], [], []
val_mse_list, val_mae_list, val_r_list = [], [], []
test_mse_list, test_mae_list, test_r_list = [], [], []


for fold_number in range(5):
    
    ############### Define Graph Deep Learning Network ##########################
    model = CustomNetworkWithGARO(opt.indim,opt.ratio,opt.nclass,opt.n_layers,opt.n_fc_layers,opt.n_clustered_communities,opt.nroi).to(device)
    ...
    tr_index,val_index,te_index = train_val_test_split(fold=fold_number, n_subjects = dataset.data.y.shape[0])
    ...
    for epoch in range(0, num_epoch):
        since  = time.time()
        tr_loss, s1_arr, s2_arr, w1, w2 = train(epoch, train_loader= train_loader, train_dataset= train_dataset)

        train_metrics = evaluate_model(model, train_loader,device)
        val_metrics = evaluate_model(model, val_loader, device)
        #scheduler.step()
        print(
                'Fold: {:03d}, Epoch: {:03d}, Loss: {:.7f} / {:.7f}, R2: {:.4f} / {:.4f}, R: {:.4f} / {:.4f}, MSE: {:.4f} / {:.4f}, MAE: {:.4f} / {:.4f}'
                ''.format(fold_number, epoch, train_metrics['loss'], val_metrics['loss'],
        ...
        log_dict = {
                f'fold_number': fold_number, f'train_loss': train_metrics['loss'], f'val_loss': val_metrics['loss'],
    ...
    test_r_list.append(test_metrics['r'])
    print('Test Stats For Fold:' + str(fold_number)+ 'MSE: ' + str(test_metrics["mse"]) + " MAE:" + str(test_metrics["mae"]) + " Loss: "+ str(test_metrics["loss"]) + " R: "+ str(test_metrics["r"]) + " R2:" + str(test_metrics["r2"]) )
    log_dict = {'test_r': str(test_metrics['r']), 'test_loss': str(test_metrics['loss']), 'test_mse': str(test_metrics["mse"]), 'test_mae': str(test_metrics["mae"])}
    wandb.log(log_dict)



    # calculate the mean and variance for each metric across all folds
    train_mse_mean = sum(train_mse_list) / len(train_mse_list)
    train_mse_variance = np.var(train_mse_list)

    train_mae_mean = sum(train_mae_list) / len(train_mae_list)
    train_mae_variance = np.var(train_mae_list)

    train_r_mean = sum(train_r_list) / len(train_r_list)
    train_r_variance = np.var(train_r_list)

    val_mse_mean = sum(val_mse_list) / len(val_mse_list)
    val_mse_variance = np.var(val_mse_list)

    val_mae_mean = sum(val_mae_list) / len(val_mae_list)
    val_mae_variance = np.var(val_mae_list)

    val_r_mean = sum(val_r_list) / len(val_r_list)
    val_r_variance = np.var(val_r_list)

    test_mse_mean = sum(test_mse_list) / len(test_mse_list)
    test_mse_variance = np.var(test_mse_list)

    test_mae_mean = sum(test_mae_list) / len(test_mae_list)
    test_mae_variance = np.var(test_mae_list)

    test_r_mean = sum(test_r_list) / len(test_r_list)
    test_r_variance = np.var(test_r_list)

# Print mean and variance for each metric
print("MSE - Train Mean: {:.2f}, Train Variance: {:.2f}".format(train_mse_mean, train_mse_variance))
print("MSE - Test Mean: {:.2f}, Test Variance: {:.2f}".format(test_mse_mean, test_mse_variance))
print("MSE - Validation Mean: {:.2f}, Validation Variance: {:.2f}".format(val_mse_mean, val_mse_variance))

print("MAE - Train Mean: {:.2f}, Train Variance: {:.2f}".format(train_mae_mean, train_mae_variance))
print("MAE - Test Mean: {:.2f}, Test Variance: {:.2f}".format(test_mae_mean, test_mae_variance))
print("MAE - Validation Mean: {:.2f}, Validation Variance: {:.2f}".format(val_mae_mean, val_mae_variance))

print("R - Train Mean: {:.2f}, Train Variance: {:.2f}".format(train_r_mean, train_r_variance))
print("R - Test Mean: {:.2f}, Test Variance: {:.2f}".format(test_r_mean, test_r_variance))
print("R - Validation Mean: {:.2f}, Validation Variance: {:.2f}".format(val_r_mean, val_r_variance))

with open("sero_rgcn_scores_garo.txt", "w") as f:
    f.write("MSE - Train Mean: {:.2f}, Train Variance: {:.2f}\n".format(train_mse_mean, train_mse_variance))
    f.write("MSE - Test Mean: {:.2f}, Test Variance: {:.2f}\n".format(test_mse_mean, test_mse_variance))
    f.write("MSE - Validation Mean: {:.2f}, Validation Variance: {:.2f}\n".format(val_mse_mean, val_mse_variance))

    f.write("MAE - Train Mean: {:.2f}, Train Variance: {:.2f}\n".format(train_mae_mean, train_mae_variance))
    f.write("MAE - Test Mean: {:.2f}, Test Variance: {:.2f}\n".format(test_mae_mean, test_mae_variance))
    f.write("MAE - Validation Mean: {:.2f}, Validation Variance: {:.2f}\n".format(val_mae_mean, val_mae_variance))

    f.write("R - Train Mean: {:.2f}, Train Variance: {:.2f}\n".format(train_r_mean, train_r_variance))
    f.write("R - Test Mean: {:.2f}, Test Variance: {:.2f}\n".format(test_r_mean, test_r_variance))
    f.write("R - Validation Mean: {:.2f}, Validation Variance: {:.2f}\n".format(val_r_mean, val_r_variance))
