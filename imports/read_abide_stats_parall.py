...
def read_sigle_data(data_dir,filename,use_gdc =False):
    ...
    corr_arr = np.copy(temp['corr'][()].reshape(53,53))

    num_nodes = pcorr.shape[0] #100 nodes
    G = from_numpy_matrix(pcorr) #Matrix
    corr_arr = corr_arr.reshape(53,53)
    G = create_thresholded_graph(corr_arr, threshold=10, num_nodes=53)
    ...
    edge_index, edge_att = coalesce(edge_index, edge_att, num_nodes,
                                    num_nodes)
    att = temp['corr'][()].reshape(53,53) #Attributes
