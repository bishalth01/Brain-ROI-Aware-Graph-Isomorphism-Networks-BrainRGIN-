...
##########################################################################################################################
class CustomNetworkWithOrthogonality(torch.nn.Module):
    def __init__(self, indim, ratio, nclass, n_hidden_layers, n_fc_layers, k, R=53, reg= 0.1):
        ...
        '''
        super(CustomNetworkWithOrthogonality, self).__init__()
...
# Define Gram-Schmidt process function
def gram_schmidt(vectors):
...
#     #basis = []
#     basis = torch.empty(0, device='cuda')
#     basis = empty_tensor.tolist()
#     for v in vectors:
#         w = v - torch.sum(torch.mv(basis, v) * basis, dim=0)
#         if torch.norm(w) > 1e-10:
#             basis.append(w)
#     return torch.stack(basis) / torch.norm(torch.stack(basis), dim=1, keepdim=True)
