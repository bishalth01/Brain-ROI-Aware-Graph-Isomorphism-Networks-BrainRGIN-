...
class GraphRepresentation(torch.nn.Module):
    ...
    def get_convs(self):
        ...
        for _ in range(self.args.num_convs):
            ...
            elif self.args.conv == 'BGCNNConv':
                if(_==0):
                    allnns.append(nn.Sequential(nn.Linear(self.R, self.k, bias=False), nn.ReLU(), nn.Linear(self.k, n_hidden_layers[_] * self.indim)))
                    convs.append(MyNNConv(self.indim, n_hidden_layers[_], allnns[_], normalize=False))


                #self.pnaconvs.append(MyPNAConv())
                else:
                    allnns.append(nn.Sequential(nn.Linear(self.R, self.k, bias=False), nn.ReLU(), nn.Linear(self.k, n_hidden_layers[_] * n_hidden_layers[_-1])))
                    convs.append(MyNNConv(n_hidden_layers[_-1], n_hidden_layers[_], allnns[_], normalize=False))
          

            if self.args.conv != "BGCNNConv":
                convs.append(conv)
...
class GraphMultisetTransformer(GraphRepresentation):
    ...
    def __init__(self, args):
        ...
        self.convs, self.allnns = self.get_convs()
    ...
    def forward(self, x, edge_index, edge_attr, batch, pos):
        ...
        for _ in range(self.args.num_convs):
            ...
            #x = F.relu(self.convs[_](x, edge_index, edge_attr, pos))
            if self.args.conv == 'BGCNNConv':
                x = F.relu(self.convs[_](x, edge_index, edge_attr, pos))
            else:
