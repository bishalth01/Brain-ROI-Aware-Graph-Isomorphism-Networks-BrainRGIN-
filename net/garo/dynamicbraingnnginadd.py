...
##########################################################################################################################
class CustomNetworkWithGINAddGARO(torch.nn.Module):
    def __init__(self, indim, ratio, nclass, n_hidden_layers, n_fc_layers, k, R=53, reg= 0.1):
        '''
        ...
        '''
        super(CustomNetworkWithGINAddGARO, self).__init__()
        ...
        for i in range(len(n_hidden_layers)):
            if(i==0):
                ...
                self.allconvs.append(MyGINConv(self.indim, n_hidden_layers[i], self.allnns[i], normalize=False))
                ...
                self.allconvs.append(MyGINConv(n_hidden_layers[i-1], n_hidden_layers[i], self.allnns[i], normalize=False))
                self.allpools.append(TopKPooling(n_hidden_layers[i], ratio=ratio, multiplier=1, nonlinearity=torch.sigmoid))
                #self.gmts.append(GraphMultisetTransformer(n_hidden_layers[i] , 128, n_hidden_layers[i] , num_nodes=R))
                self.garos.append(ModuleGARO(output_dim=n_hidden_layers[i] , hidden_dim=n_hidden_layers[i] , dropout=0.1, upscale=1.0))
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        for i in range(len(self.n_hidden_layers)):
            ...
            #Interpretation
            perm_reshaped = (perm.reshape(64, 25) % 53).cuda()
            score_reshaped = score.reshape(64,-1)

            topk_indices = perm_reshaped

            # Compute the frequency of each node being selected among the batches
            node_counts = torch.zeros(53, device="cuda")  # Assuming 42 is the number of nodes
            for batch_indices in topk_indices:
                unique_indices, counts = torch.unique(batch_indices, return_counts=True)
                node_counts[unique_indices] += counts

            # Report nodes with frequency of 100% or greater than 90%
            total_batches = topk_indices.size(0)
            threshold_100 = total_batches
            threshold_90 = total_batches * 0.9
            threshold_95 = total_batches * 0.95

            print("Nodes with a frequency of 100%:")
            for node_index, count in enumerate(node_counts):
                if count.item() == threshold_100:
                    #node_number = torch.argmax(perm_reshaped[:, node_index]).item()
                    print(f"Node {node_index}: {count.item()} times selected.")
                    #print(f"Node {node_index} (Node Number: {node_number}): {count.item()} times selected.")
            
            print("Nodes with a frequency greater than 95%:")
            for node_index, count in enumerate(node_counts):
                if count.item() > threshold_95:
                    #node_number = torch.argmax(perm_reshaped[:, node_index]).item()
                    print(f"Node {node_index}: {count.item()} times selected.")
                    #print(f"Node {node_index} (Node Number: {node_number}): {count.item()} times selected.")
                    #print(f"Nodes are {node_index}")

            print("Nodes with a frequency greater than 90%:")
            for node_index, count in enumerate(node_counts):
                if count.item() > threshold_90:
                    #node_number = torch.argmax(perm_reshaped[:, node_index]).item()
                    print(f"Node {node_index}: {count.item()} times selected.")
                    #print(f"Node {node_index} (Node Number: {node_number}): {count.item()} times selected.")

             #Interpretation
            # perm_reshaped = perm.reshape(64, 25)
            # # Flatten the perm tensor to get a 1D tensor of positions
            # flattened_perm = perm_reshaped.flatten()
            # #x_gmt  = self.gmts[i](x, index= batch,edge_index = edge_index)
            # # Count the occurrences of each position in the flattened_perm tensor
            # position_counts = torch.bincount(flattened_perm)

            # # Find the top 5 positions that are highly recurring across all subjects
            # top5_positions = torch.topk(position_counts, k=5).indices

            # perm_reshaped = perm.reshape(64, 25) % 53
            # score_reshaped = score.reshape(64,-1)

            # # Flatten perm_reshaped to create a 1D tensor
            # flattened_perm = perm_reshaped.flatten()
            ...
            # average_scores = np.zeros(num_nodes)
            # counts = np.zeros(num_nodes, dtype=int)
            # for i in range(perm_reshaped.shape[0]):
            #     for j in range(perm_reshaped.shape[1]):
            #         node = perm_reshaped[i, j]
            #         average_scores[node] += score_reshaped[i, j]
            #         counts[node] += 1

            # average_scores /= len(counts)

            # # Sort the nodes based on their average scores in descending order
            # sorted_indices = np.argsort(average_scores)[::-1]

            # # Select the top 3 nodes with the highest average scores
            # top15_nodes = sorted_indices[:15]

            # # Calculate the occurrence frequencies of each node
            # node_counts = torch.bincount(flattened_perm)

            # # Sort the nodes based on their scores in descending order
            # sorted_indices = torch.argsort(score_reshaped.mean(dim=0), descending=True)

            # # Select the top 15 nodes with the highest scores, considering their occurrence across the subjects
            # top15_nodes = []
            # for idx in sorted_indices:
            #     if len(top15_nodes) >= 15:
            #         break
            #     node = idx.item()
            #     if node_counts[node] > 0:
            #         top15_nodes.append(node)


            #x_gmt  = self.gmts[i](x, index= batch,edge_index = edge_index)
            garo_output,_ = self.garos[i](x, batch)
