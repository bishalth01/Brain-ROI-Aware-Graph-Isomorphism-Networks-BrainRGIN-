...
class CustomNetworkWithGARO(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        for i in range(len(self.n_hidden_layers)):
            ...
            #Interpretation
            perm_reshaped = (perm.reshape(64, 42) % 53).cuda()
            score_reshaped = score.reshape(64,-1)

            # Calculate the number of nodes to select (top 50%)
            ...
            # final_nodes_to_select = int(0.70 * 53)
            # #num_nodes_to_select = int(perm_reshaped.size(1) * final_nodes_to_select)
            # #num_nodes_to_select = perm_reshaped.size(1) 
            # num_nodes_to_select = final_nodes_to_select

            # # Get the indices of the top-k nodes for each batch
            # topk_indices = torch.topk(perm_reshaped, num_nodes_to_select, dim=1).indices
