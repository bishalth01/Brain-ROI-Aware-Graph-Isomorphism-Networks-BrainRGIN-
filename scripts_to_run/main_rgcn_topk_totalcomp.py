...
wandb.init(project='results_abcd_rgcn_topk_mean', save_code=True, config="/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/wandb_sweeps/default_test_sweep_totalcomp_bgnn.yaml")
...
# #-----TESTING
# model.load_state_dict(torch.load("/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/model/0totalcomp_braingnn_final.pth"))
# model.eval()
...
for epoch in range(0, num_epoch):
    ...
    if val_loss < best_loss and epoch > 5:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        if save_model:
            torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+'totalcomp_braingnn_publication'+'.pth'))
