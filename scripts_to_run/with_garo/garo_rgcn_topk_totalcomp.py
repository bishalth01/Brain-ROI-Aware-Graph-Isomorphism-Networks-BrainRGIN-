...
from net.garo.dybamicbraingnnmean import CustomNetworkWithGARO
...
wandb.init(project='results_abcd_rgcn_topk_mean', save_code=True, config="/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/wandb_sweeps/default_test_sweep_totalcomp.yaml")
...
dataset = ABIDEDatasetTotalComp(path,name)
...
model = CustomNetworkWithGARO(opt.indim,opt.ratio,opt.nclass,opt.n_layers,opt.n_fc_layers,opt.n_clustered_communities,opt.nroi,opt.reg).to(device)
...
#-----TESTING
model.load_state_dict(torch.load("/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/model/0totalcomp_perfect_model.pth"))
model.eval()
...
reshaped_allgrads = allgrads.reshape(opt.batchSize, 53,53)
...
for epoch in range(0, num_epoch):
    ...
    if val_loss < best_loss and epoch > 5:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        if save_model:
            torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+'totalcomp_perfect_model'+'.pth'))
