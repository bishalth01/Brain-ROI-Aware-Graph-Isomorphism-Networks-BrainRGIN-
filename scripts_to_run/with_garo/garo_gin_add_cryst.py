...
#from net.dynamicbrainwithGMT import CustomNetworkWithGMT, CustomNetworkWithGINAddGARO
...
wandb.init(project='results_abcd_gin_topk_add', save_code=True, config="/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/wandb_sweeps/default_test_sweep_crystallized.yaml")
...
#-----TESTING
model.load_state_dict(torch.load("/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/model/0crystallized_best_model.pth"))
model.eval()
...
for epoch in range(0, num_epoch):
    ...
    if val_loss < best_loss and epoch > 5:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        if save_model:
            torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+'crystallized_best_model'+'.pth'))
