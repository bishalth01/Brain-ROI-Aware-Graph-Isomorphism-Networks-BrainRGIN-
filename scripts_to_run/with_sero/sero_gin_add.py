...
from net.sero.dynamicbraingnnginadd import CustomNetworkGINSeroAdd
import numpy as np
import argparse
import time
import copy
...
model = CustomNetworkGINSeroAdd(opt.indim,opt.ratio,opt.nclass,opt.n_layers,opt.n_fc_layers,opt.n_clustered_communities,opt.nroi,opt.reg).to(device)
...
# #-----TESTING
# model.load_state_dict(torch.load("/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/model/0fluid_best_model.pth"))
# model.eval()
...
# np.save("FluidPredictionGrads.npy",summed_grads)
# sys.exit()
...
for epoch in range(0, num_epoch):
    ...
    if val_loss < best_loss and epoch > 5:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        if save_model:
            torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+'fluid_best_model'+'.pth'))
...
#summed_grads.save("FinalSailentGrads.npy", allow_pickle=True)
np.save("FinalFluidPredictionModelGrads.npy",summed_grads)
sys.exit()