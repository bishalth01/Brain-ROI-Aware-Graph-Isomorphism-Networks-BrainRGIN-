...
wandb.init(project='results_abcd_rgcn_topk_mean', save_code=True, config="/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/wandb_sweeps/default_test_sweep_crystallized_bgnn.yaml")
...
dataset = ABIDEDatasetCryst(path,name)
...
for epoch in range(0, num_epoch):
    ...
    if val_loss < best_loss and epoch > 5:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        if save_model:
            torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+'crystallized_braingnn_final'+'.pth'))
