...
wandb.init(project='results_abcd_rgcn_topk_mean', save_code=True, config="/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/wandb_sweeps/default_test_sweep.yaml")
#wandb.init(project='results_abcd_rgcn_topk_mean', save_code=True, config="/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/wandb_sweeps/cross_validation/abcd_rgcn.yaml")
...
tr_index,val_index,te_index = train_val_test_split(fold=fold, n_subjects = dataset.data.y.shape[0] )
...
model = CustomNetwork(opt.indim,opt.ratio,opt.nclass,opt.n_layers,opt.n_fc_layers,opt.n_clustered_communities,opt.nroi,opt.reg).to(device)
...
#-----TESTING
# model.load_state_dict(torch.load("/data/users2/bthapaliya/IntelligenceABCDPredictionBrainGNN/model/0fluid_best_final_model.pth"))
# model.eval()
...
# np.save("FluidIntelligenceGrads.npy",summed_grads)
# sys.exit()
...
for epoch in range(0, num_epoch):
    ...
    if val_loss < best_loss and epoch > 5:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        if save_model:
            torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+'fluid_best_final_model'+'.pth'))
